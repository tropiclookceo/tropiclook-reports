...
# Recurring monthly external bills где забыть оплату = инцидент перед собственником.
# При budget>0 и fact=0 ставится "🔴 НЕ ОПЛАЧЕНО" (а не ✅ как раньше).
# Управление per-property через budget_amount: =0 значит TL не платит этот месяц
# (собственник сам / включено в condo-fee) — check выключается автоматически.
# Интернет (FIX-INET) сюда НЕ входит: оплачивается раз в год → отдельный annual renewal watch.
RECURRING_MANDATORY = {"UTL-ELEC"}
...
def _validate_input_workbook_shape(wb, workbook_label="workbook"):
    missing = [name for name in REQUIRED_INPUT_SHEETS if name not in wb.sheetnames]
    if not missing:
        return
    ...
    msg.append("expected TL_[CODE]_InputData_YYYY-MM.xlsx with 8 required sheets")
...
def build_opex_passport(wb, data, rpt_year, rpt_month):
    ...
    for cat in all_cats:
        ...
        if cat in RECURRING_MANDATORY and bgt > 0 and fact == 0:
            status = "🔴 НЕ ОПЛАЧЕНО"
        elif bgt == 0 and fact > 0:
